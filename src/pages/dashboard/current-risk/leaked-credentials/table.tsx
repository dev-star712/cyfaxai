...
const EmailProtectionResultsAllTable = ({ data }: { data: TableData }) => {  
  ...
  const tableBody = data ?   
    data.map((row, i) => (  
      <tr key={i} className={`[&>td]:border-b [&>td]:py-4 [&>td]:pl-6 [&>td]:font-mulish [&>td]:text-xs [&>td]:first:pl-[16px] sm:[&>td]:text-sm sm:[&>td]:first:pl-6 ${!isHomePage ? 'even:bg-[#ebe8e8] even:text-black' : ''}`}>  
